...
> **状态**：v1.0 交付候选正文。团队 5 人分工已落实（见下表）；指导老师待补；本文件与 `submissions/项目详细方案.md`（44KB 完整版）互为总-分关系。

## 团队角色与分工（对应 A10.md 十七节）

| 成员 | 角色 | 负责内容（本项目实际分工） | 在方案中的作用 |
| --- | --- | --- | --- |
| 张晟林 | 需求分析 | 赛题需求拆解、赛题合规对照、用户场景与服务模型、S2A/S2B 业务分析 | 确保方案对齐赛题评分点与真实教学痛点，需求—交付闭环 owner |
| 肖钰涛 | 架构设计 | 系统分层、技术选型（Next.js 15 / Neo4j / ECharts / Vercel AI SDK）、图存储双引擎抽象、S2C 技术路线 | 技术方案总负责人，决定可扩展性与工程边界 |
| 张旭业 | 编码与 demo 实现 | 前后端与 AI 抽取/RAG 编码、图谱可视化、教师端/学生端页面、离线演示数据集、可跑通 demo | 把架构落地为可运行系统，直接决定评审第一印象 |
| 桑东杰 | 测试 | vitest 单测、Playwright e2e、多浏览器冒烟、性能实测脚本、准确率与问答测试集 | 端到端可信度与赛题性能/准确率指标的取证者 |
| 周靖超 | 审计 | 安全审计（OWASP Top 10）、数据隐私声明、依赖与合规审查、PR code review | 工程合规与安全纵深把关人 |

（浙江师范大学 A10 参赛队，5 人，未设正式队名；一人多岗、互为 backup reviewer；指导老师待团队补充。）
...
## S2B：组织管理与业务分析方案（主要由需求分析 张晟林 负责）

**团队组织结构**：5 人（需求分析 / 架构设计 / 编码实现 / 测试 / 审计 各 1），按角色矩阵式分工（见上表）；规模精简，一人多岗、互为 backup reviewer。
...
- **决策机制**：技术选型由架构设计（肖钰涛）拍板（有争议时 ADR 记录）；进度与范围由需求分析（张晟林）统筹；重大变更走 PR review。
...
**负责人**：张晟林（需求分析，S2B owner）。

## S2C：技术路线及实现方案（主要由架构设计 肖钰涛 负责）
...
**负责人**：肖钰涛（架构设计，S2C owner）。

## S2D：成本模型及可行性分析（主要由需求分析 张晟林 负责，审计 周靖超 协作）
...
**负责人**：张晟林（需求分析，S2D owner）· 周靖超（审计，合规与成本审阅）。
